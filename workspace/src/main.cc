...
/**
 * @brief Run DPU Task for Unet
 *
 * @param 
 *
 * @return none
 */
void runUnet(ai::DpuRunner* runner) {
    ...
    for (unsigned int n = 0; n < images.size(); n+=batchSize) {
              ...
              for (int c =0; c< 3; c++)
                for (int h =0; h< inHeight;h++)
                  for (int w =0; w< inWidth;w++)
                    imageInputs[i*inSize 
                      + (c*inHeight*inWidth) + (h*inWidth) + w] 
        ...
        batchTensors.push_back(std::shared_ptr<ai::Tensor>(new ai::Tensor(inputTensors[0]->get_name(),in_dims,inputTensors[0]->get_data_type())));
                
        inputs.push_back(ai::CpuFlatTensorBuffer(imageInputs,batchTensors.back().get()));
        batchTensors.push_back(std::shared_ptr<ai::Tensor>(new ai::Tensor(outputTensors[0]->get_name(),out_dims,outputTensors[0]->get_data_type())));
        outputs.push_back(ai::CpuFlatTensorBuffer(FCResult,batchTensors.back().get()));
...
/**
 * @brief Entry for runing Unet neural network
 *
 * @note DNNDK APIs prefixed with "dpu" are used to easily program &
 *       deploy Unet50 on DPU platform.
 *
 */
