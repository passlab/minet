...
class Node(XstreamNode):
  def initialize(self, args):
    ...
    for si in range(self._bsz * self._numStreamBuffers):
      ...
      self._outputBuffers.append(np.empty(
        (self._bsz,) + tuple (firstOutputShape[1:]),
  ...
  def wait(self, q):
    ...
    while True:
      try:
        ...
        self.put(meta, self._outputBuffers[sId], pub=pub)
      except Exception as e:
        logger.error(e)
  ...
  def run(self):
    ...
    firstOutputName = next(iterkeys(self._compJson.getOutputs()))
    ...
    while True:
      try:
        ...
        self._fpgaRT.exec_async( \
          ...
          {firstOutputName: self._outputBuffers[sId]}, sId)
